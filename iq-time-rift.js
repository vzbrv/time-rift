(() => {
  ...
  const themes = [
    {
      ...
      featurePath: {
        ...
        label: "Lores from IQ.wiki wikis",
        description: "Timeline lores built from IQ.wiki wiki pages on crypto and DeFi.",
  ...
  constructor() {
    ...
    this.showTutorial = !tutorialSeen;
  }
      ...
      this.shadowRoot.innerHTML = `
        ...
        <section class="rift-shell">
          ...
          <div class="stat-row" aria-label="Game features">
            ...
            <span>Lores from IQ.wiki wikis</span>
          </div>
        </section>
    ...
    themeHeader() {
      ...
      return `
        <span class="eyebrow">${esc(featurePath?.label || "Lores from IQ.wiki wikis")}</span>
    ...
    featurePathMarkup(path) {
      return `
        <div class="feature-path">
          ...
          <p>${esc(path.description || "Timeline lores built from IQ.wiki wiki pages.")}</p>
    ...
    previewMarkup() {
      return `
        ...
        <div class="preview-count">${this.theme.events.length} timeline events · ${esc(this.difficultySummary())}</div>
        ...
        <div class="locked-grid">
          ${[1, 2, 3, 4].map((n) => `<div class="locked-card"><b>Event ${String(n).padStart(2, "0")}</b><span>Order hidden</span></div>`).join("")}
        </div>
      `;
    }
          ...
          this.state.phase === "revealing"
            ...
            : `
              <div class="round-head">
                ...
                <h3>${plan.type === "boss" ? "Rebuild the full path from earliest to latest." : "Place this event where it belongs in the timeline."}</h3>
    ...
    placeMarkup(plan) {
      ...
      return `
        ...
        <div class="candidate ${rift ? "rift-shake" : ""}" aria-live="polite">
          ...
          <small>${rift ? "Timeline break detected - try a different slot." : "Tap a slot to place it."}</small>
        </div>
      `;
    }
          ...
          ${plan.anchors.map((id, index) => {
            ...
            return `
              <button class="event-card ${misplaced && rift ? "rift-card" : ""}" data-corrupt="${esc(id)}">
                ...
                <small>${misplaced && rift ? "Misplaced" : "Year hidden"}</small>
              </button>
              ${index < plan.anchors.length - 1 ? '<i class="connector"></i>' : ""}
            `;
          }).join("")}
        </div>
        <p class="micro">One event is drifting out of order. Find the corrupted card.</p>
      `;
    }
    ...
    bossMarkup() {
      ...
      return `
        <div class="boss-panel">
          ...
          <p class="boss-tip">Grab the <b>⋮⋮</b> handle to drag a row, or tap Earlier/Later. Correct rows anchor (✓), reveal their date, and lock in place.</p>
    ...
    gapButton(index, rift, armed = true) {
      return `
        <button class="gap portal ${rift?.gap === index ? "rift-hit" : ""} ${armed ? "armed" : "disabled"}" data-gap="${index}" ${armed ? "" : "disabled"} aria-label="Place event in slot ${index + 1}">
          <i></i><span>${rift?.gap === index ? "Break" : "Slot"}</span>
        </button>
      `;
    }
    ...
    openModal() {
      ...
      setTimeout(() => this.shadowRoot.querySelector(".modal button, .modal [href]")?.focus(), 0);
    }
    ...
    handleKeys(event) {
      ...
      const focusables = [...this.shadowRoot.querySelectorAll(".modal button:not([disabled]), .modal a, .modal textarea")];
    ...
    styles() {
      return `
        :host{display:block;color:#FAFCF8;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;--iq-pink:#FF1A88;--iq-pink-light:#FF5CAA;--iq-navy:#17202B;--iq-navy-dark:#0F172A;--iq-blue:#272D38;--iq-off:#FAFCF8;--iq-muted:#F3F4F6;--iq-white:#FFFFFF}
        *{box-sizing:border-box}
        button,a,textarea,select{font:inherit}
        button{cursor:pointer}
        button:disabled{cursor:not-allowed;opacity:.58}
        .rift-shell{position:relative;overflow:hidden;border-radius:18px;min-height:640px;padding:34px;background:linear-gradient(90deg,rgba(15,23,42,.98) 0%,rgba(15,23,42,.92) 35%,rgba(15,23,42,.48) 68%,rgba(15,23,42,.70) 100%),url("./assets/iq-time-rift-rift-art.png") center/cover no-repeat,var(--iq-navy-dark);box-shadow:0 24px 80px rgba(15,23,42,.48)}
        .rift-shell::before{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,26,136,.16) 1px,transparent 1px),linear-gradient(0deg,rgba(243,244,246,.08) 1px,transparent 1px);background-size:72px 72px;mask-image:linear-gradient(90deg,rgba(0,0,0,.42),rgba(0,0,0,.10) 42%,transparent 76%);pointer-events:none}
        ...
        .hero{position:relative;display:grid;grid-template-columns:minmax(280px,.82fr) minmax(340px,1.18fr);gap:32px;align-items:stretch;min-height:500px;z-index:1}
        .hero-copy{align-self:center;max-width:620px;padding:26px;border-left:4px solid var(--iq-pink);background:linear-gradient(90deg,rgba(15,23,42,.82),rgba(23,32,43,.46));box-shadow:inset 0 0 0 1px rgba(255,255,255,.10)}
        ...
        h1,h2,h3,p{margin:0}
        h1{max-width:760px;margin-top:14px;font-size:clamp(40px,6vw,76px);line-height:.92;letter-spacing:0;text-wrap:balance}
        ...
        .select-control{display:grid;gap:6px;min-width:170px}
        .select-control span{font-size:12px;font-weight:900;color:var(--iq-pink-light);text-transform:uppercase}
        .select-control select{width:100%;min-height:48px;border-radius:8px;padding:0 12px;background:rgba(15,23,42,.68);color:var(--iq-white);border:1px solid rgba(255,255,255,.18);font-weight:850}
        ...
        .preview{align-self:end;position:relative;min-height:430px;padding:24px;border:1px solid rgba(255,255,255,.18);border-radius:12px;background:linear-gradient(180deg,rgba(15,23,42,.34),rgba(15,23,42,.82));backdrop-filter:blur(8px);box-shadow:inset 0 0 0 1px rgba(255,26,136,.16),0 24px 52px rgba(0,0,0,.28)}
        ...
        .locked-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
        .locked-card{padding:18px;border-radius:8px;background:linear-gradient(135deg,rgba(255,255,255,.11),rgba(255,255,255,.035));border:1px solid rgba(255,255,255,.14);filter:blur(.15px)}
        .locked-card b,.locked-card span{display:block}.locked-card span{color:rgba(243,244,246,.62);margin-top:8px}
        ...
        .modal{position:fixed;inset:3vh max(18px,calc((100vw - 1060px)/2));z-index:40;overflow:auto;border-radius:24px;background:var(--iq-navy);border:1px solid rgba(255,255,255,.16);box-shadow:0 30px 90px rgba(0,0,0,.55);animation:rise .24s ease-out}
        .modal-head{position:sticky;top:0;z-index:2;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:22px 24px;background:rgba(23,32,43,.88);backdrop-filter:blur(18px);border-bottom:1px solid rgba(255,255,255,.12)}
        ...
        .progress-block{margin-bottom:22px}.progress-copy{display:flex;justify-content:space-between;color:var(--iq-muted);margin-bottom:10px}.stability{height:10px;border-radius:999px;background:rgba(255,255,255,.10);overflow:hidden}.stability i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--iq-pink),var(--iq-pink-light));box-shadow:0 0 18px rgba(255,26,136,.78)}.blocks{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-top:10px}.blocks b{height:8px;border-radius:999px;background:rgba(255,255,255,.14)}.blocks .on{background:var(--iq-pink-light)}
        ...
        .line-glow{position:absolute;left:24px;right:24px;top:50%;height:4px;background:linear-gradient(90deg,var(--iq-pink),var(--iq-pink-light),var(--iq-pink));box-shadow:0 0 24px rgba(255,26,136,.60);opacity:.72}
        .timeline-board:not(.armed) .line-glow{opacity:.32;filter:saturate(.7)}
        .timeline-board.armed .gap{box-shadow:0 0 22px rgba(255,26,136,.24)}
        .timeline-board.cracked .line-glow{background:linear-gradient(90deg,var(--iq-pink),var(--iq-pink-light) 45%,var(--iq-blue));animation:crack .45s ease 2}
        ...
        .gap i{width:28px;height:28px;border-radius:50%;border:2px solid var(--iq-pink-light);box-shadow:0 0 18px var(--iq-pink-light)}.gap span{font-size:11px;font-weight:900;text-transform:uppercase}
        ...
        @media (max-width:760px){
          .rift-shell{padding:18px;border-radius:0;min-height:100vh;background-position:62% center}.hero{grid-template-columns:1fr;min-height:auto;gap:20px}.hero-copy{padding:20px}.preview{min-height:320px;align-self:auto}h1{font-size:44px}.select-control,.source-select{width:100%}.schematic-rift{height:72px}.stat-row{grid-template-columns:1fr}.modal{inset:0;border-radius:0}.game-slot{padding:18px}.timeline-board{align-items:stretch;flex-direction:column;overflow:visible}.line-glow{top:28px;bottom:28px;left:50%;right:auto;width:4px;height:auto}.event-card,.candidate,.gap{width:100%;min-width:0}.gap{height:68px;border-radius:16px}.locked-grid,.result-grid{grid-template-columns:1fr}.boss-row{grid-template-columns:32px 1fr}.mobile-move{grid-column:1 / -1}.sticky-actions{margin-left:-18px;margin-right:-18px;padding:14px 18px}.restored-path div{grid-template-columns:82px 1fr}.term-row{grid-template-columns:1fr;gap:4px}
        }
      `;
    }
  }
